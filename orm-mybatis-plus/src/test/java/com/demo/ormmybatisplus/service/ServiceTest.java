package com.demo.ormmybatisplus.service;
...
import com.demo.ormmybatisplus.OrmMybatisPlusApplicationTests;
 ...
 */
@Slf4j
public class ServiceTest extends OrmMybatisPlusApplicationTests {
    @Autowired
    ...
    public void testSave() {
        ...
        User testSave3 = User.builder().name("testSave3").password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave3@swaggy.com").phoneNumber("17300000003").status(1).lastLoginTime(new DateTime()).build();
    ...
    public void testSaveList() {
        ...
        for (int i = 4; i < 14; i++) {
            String salt = IdUtil.fastSimpleUUID();
            User user = User.builder().name("testSave" + i).password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave" + i + "@swaggy.com").phoneNumber("1730000000" + i).status(1).lastLoginTime(new DateTime()).build();
            userList.add(user);
        }
